...
class SpanAeTest(ModelTestCase):
    def setUp(self):
        super(SpanAeTest, self).setUp()
        self.set_up_model("tests/fixtures/experiment.json",
                          "tests/fixtures/parallel_copy.tsv")
    ...
    def test_span_modules_initialized(self):
        assert issubclass(type(self.model._span_extractor), SpanExtractor)
        assert issubclass(type(self.model._span_pruner), SpanPruner)
