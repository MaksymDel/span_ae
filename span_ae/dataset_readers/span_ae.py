...
@DatasetReader.register("span_ae")
class SpanAeDatasetReader(DatasetReader):
    """
    Read a tsv file containing paired sequences, and create a dataset suitable for a
    ``SpanAe`` model, or any model with a matching API.
    ...
    def text_to_instance(self, source_string: str, target_string: str = None) -> Instance:  # type: ignore
        ...
        source_spans_indeces = enumerate_spans(tokenized_source, max_span_width=self._max_span_width)
        source_spans = ListField([SpanField(i, j, source_field) for i, j in source_spans_indeces])
        if target_string is not None:
            tokenized_target = self._target_tokenizer.tokenize(target_string)
            tokenized_target.insert(0, Token(START_SYMBOL))
            tokenized_target.append(Token(END_SYMBOL))
            target_field = TextField(tokenized_target, self._target_token_indexers)
            return Instance({"source_spans": source_spans, "source_tokens": source_field, "target_tokens": target_field})
        else:
            return Instance({'source_spans': source_spans, 'source_tokens': source_field})

    @classmethod
    def from_params(cls, params: Params) -> 'SpanAeDatasetReader':
        ...
        max_span_width = params.pop('max_span_width', 1)
        params.assert_empty(cls.__name__)
        return SpanAeDatasetReader(source_tokenizer, target_tokenizer,
                                   source_token_indexers, target_token_indexers,
                                   source_add_start_token, lazy, max_span_width)
